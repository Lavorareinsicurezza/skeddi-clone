...
@section('content')
    <div class="container mx-auto px-6 py-8">
        ...
        <div class="mb-6 flex items-center justify-between">
            <h1 class="text-3xl font-bold text-gray-900">{{ __('lang.create_document_type') }}</h1>
        </div>
        ...
        <form action="{{ route('admin.document-types.store') }}" method="POST">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="block text-sm font-bold text-gray-900 mb-2">{{ __('lang.document_type') }} <span class="text-red-500">*</span></label>
                        <input type="text" name="name" placeholder="{{ __('lang.enter_document_name') }}" value="{{ old('name') }}" required
                ...
                <div class="mb-6">
                    ...
                    <textarea name="notes" placeholder="{{ __('lang.enter_notes') }}" rows="4"
                        class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('notes') }}</textarea>
                </div>
