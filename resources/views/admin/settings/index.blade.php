...
@section('content')
    ...
    <form action="{{ route('admin.settings.update') }}" method="POST" class="bg-white rounded-lg shadow-sm">
        ...
        <div class="px-6 py-3">
            ...
            <div id="whatsapp-test-message-box" class="mt-4"></div>
        </div>
    ...
    <script>
        ...
        // Test WhatsApp Business API Button Handler
        document.getElementById('testWhatsAppBtn').addEventListener('click', function () {
            const btn = this;
            const originalText = btn.innerHTML;
            ...
            fetch('{{ route('admin.settings.test-whatsapp') }}', {
                ...
                .then(data => {
                    if (data.success) {
                        // Show success message
                        const container = document.getElementById('whatsapp-test-message-box');
                        container.innerHTML = `
                                <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative" role="alert">
                                    <div class="flex items-center">
                                        <svg class="w-6 h-6 text-green-600 mr-3" fill="currentColor" viewBox="0 0 20 20">
