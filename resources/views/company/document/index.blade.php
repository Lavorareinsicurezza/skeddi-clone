        ...
        @foreach ($documents as $document)
            ...
            <div
                ...
                <div class="absolute top-3 right-3 flex space-x-0">
                    @can('edit company-documents')
                        <a href="{{ route('admin.company-documents.edit', $document->id) }}"
                            class="text-[#0C3183] hover:text-[#0A2869]"> <i class="fa fa-pencil"></i> </a>
                    @endcan
                    @can('delete company-documents')
                        <form action="{{ route('admin.company-documents.destroy', $document->id) }}" method="POST"
                            class="inline-block ml-2" onsubmit="return confirm('{{ __('lang.delete_document_confirm') }}');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-red-500 hover:text-red-600" title="{{ __('lang.actions') }}">
